    ...
    <section id="phase1" className="mb-16 scroll-mt-20">
      ...
      <div className="space-y-5 text-[#777] text-base leading-relaxed">
        <p>
          ...
          Hier machen sich die Prinzipien des <strong className="text-[#f0ede8]">Spec-Driven Development (SDD)</strong> bezahlt,
          wie sie kürzlich auch von GitHub in einem neuen Open-Source-Toolkit ("Spec Kit") hervorgehoben wurden.
        </p>

        <div className="bg-[#111] border border-[#1f1f1f] rounded-none p-5">
          <p className="text-sm font-semibold text-[#f97316] uppercase tracking-wide mb-2">Das wichtigste Mindset-Shift</p>
          <p className="text-[#f0ede8]">
            Wir bewegen uns von <span className="line-through text-[#444]">"Code ist die Quelle der Wahrheit"</span> hin zu
            <span className="text-[#f97316] font-semibold"> "Die Absicht (Intent) ist die Quelle der Wahrheit"</span>.
          </p>
        </div>

        <p>
          Wir müssen aufhören, Coding-Agenten wie Suchmaschinen zu behandeln. Wir sollten sie vielmehr als{' '}
          <strong className="text-[#f0ede8]">extrem buchstäbliche Pair-Programmer</strong> betrachten.
        ...
        <p>
          <strong className="text-[#f0ede8]">Das Ziel:</strong> Ein solides Textdokument, das kein statisches Dokument für später ist,
          sondern ein <strong className="text-[#f97316]">lebendiges, ausführbares Artefakt</strong>. Diese Spec ist der Vertrag darüber,
          wie sich deine Software verhalten soll, und dient der KI als alleinige Quelle der Wahrheit (Source of Truth).
        </p>
      </div>

      <div className="mt-10 mb-8">
        <h3 className="text-lg font-semibold text-[#f0ede8] mb-6">
          Der optimale Ablauf <span className="text-[#444] font-normal text-sm">(angelehnt an das GitHub Spec Kit)</span>
        </h3>
        <div className="space-y-4">
          {workflowSteps.map((step) => (
            <div key={step.num} className="flex gap-4 p-5 bg-[#111] border border-[#1f1f1f] rounded-none hover:border-[#f97316]/30 transition-colors duration-200">
              <span className="text-2xl font-bold text-[#1f1f1f] font-mono min-w-[40px]">{step.num}</span>
              <div>
                <p className="font-semibold text-[#f0ede8] mb-1">{step.title}</p>
      ...
      <div className="mt-8">
        <h3 className="text-lg font-semibold text-[#f0ede8] mb-4">Essenzielle Architektur-Entscheidungen</h3>
        <p className="text-[#777] text-sm mb-5">
          ...
          <span className="text-[#f97316]">System Design 101 Repository von ByteByteGo</span>.
          Hier sind die wichtigsten Architektur-Entscheidungen, die in deine Spec gehören:
        </p>
        <div className="space-y-4">
          {archItems.map((group) => (
            <div key={group.title} className="border border-[#1f1f1f] rounded-none overflow-hidden">
              <div className="px-5 py-3 bg-[#111] border-b border-[#1f1f1f]">
                <p className="text-sm font-semibold text-[#f97316]">{group.title}</p>
              </div>
              <div className="divide-y divide-[#1f1f1f]">
                {group.items.map((item) => (
                  <div key={item.name} className="px-5 py-4 bg-[#111] flex gap-3">
                    <span className="text-[#f97316] text-sm mt-0.5">•</span>
                    <div>
                      <span className="text-[#f0ede8] font-medium text-sm">{item.name}: </span>
                      <span className="text-[#777] text-sm">{item.desc}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="mt-6">
        <p className="text-sm font-semibold text-[#777] mb-2">Beispiel für den "Plan"-Teil in deiner SPEC.md:</p>
      ...
      <div className="mt-8">
        ...
        <p className="text-[#777] text-sm my-3">Oder starte mit diesem KI-Prompt (z.B. in Gemini):</p>
        <CodeBlock code={aiPrompt} language="prompt" />
      </div>

      <div className="mt-8">
        <h3 className="text-lg font-semibold text-[#f0ede8] mb-4">Wichtige Dinge, die in die Spec gehören</h3>
        <div className="space-y-2">
          {specContents.map((item, i) => (
            <div key={i} className="flex items-start gap-3 p-3 rounded-none">
      ...
      <Callout type="tip" title="Profi-Tipp">
        Speichere das finale Ergebnis als <code className="text-[#f97316] bg-[#f97316]/10 px-1.5 py-0.5 rounded font-mono text-xs">SPEC.md</code> ab.
