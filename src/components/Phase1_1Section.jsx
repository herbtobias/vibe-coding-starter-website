...
const iconColors = ['text-[#00d4ff]', 'text-[#10b981]', 'text-[#f59e0b]', 'text-[#ef4444]'];
const badgeColors = ['bg-[#00d4ff]/10 text-[#00d4ff]', 'bg-[#10b981]/10 text-[#10b981]', 'bg-[#f59e0b]/10 text-[#f59e0b]', 'bg-[#ef4444]/10 text-[#ef4444]'];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {techStacks.map((stack, i) => {
          ...
          return (
            <div
              key={stack.label}
              className={`bg-[#0d1f3c] border rounded-xl p-5 transition-all duration-300 ${accentColors[i]}`}
            >
              <div className="flex items-start gap-3 mb-4">
                <div className={`p-2 rounded-lg bg-[#0a1628] ${iconColors[i]}`}>
                  <Icon size={18} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full font-mono ${badgeColors[i]}`}>
